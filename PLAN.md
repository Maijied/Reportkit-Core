...
| Phase | Status |
|-------|--------|
| Monorepo consolidation | **Done locally** — single repo `Maijied/Reportkit-Website` |
| Website + Worker + D1 | Workflows at repo root; secrets on Website repo |
| Packagist / npm | Update package URLs to monorepo on Packagist |

## Monorepo

Everything lives in **[Maijied/Reportkit-Website](https://github.com/Maijied/Reportkit-Website)**. See [MONOREPO.md](./MONOREPO.md).
...
1. Push monorepo to `Maijied/Reportkit-Website` (replaces old site-only layout)
2. Archive split repos on GitHub
3. Update Packagist package source URLs to the monorepo
4. Ensure all secrets are on **Reportkit-Website** (not Reportkit-UI)
