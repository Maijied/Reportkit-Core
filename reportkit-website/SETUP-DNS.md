...
> **Future automation (separate repo):** See [DOMAIN-CONTROL-PANEL-INSTRUCTIONS.md](./docs/DOMAIN-CONTROL-PANEL-INSTRUCTIONS.md) for blueprint instructions to build **Lorapok Domain Hub** — a standalone site (Google Sign-In) to manage Cloudflare, DNS, GitHub Pages/Actions, and Workers. **Not implemented in Reportkit-Core.**
...
| Worker | `reportkit-api` | Cloudflare Worker `reportkit-demo-api` | **Cloudflare only** — after nameserver migration (see below) |

**Do not add `reportkit-api` at get.tech** while nameservers still point to get.tech. Worker custom domains require Cloudflare to control DNS for `lorapok.tech`.

> **SSL note (Free plan):** Use **`reportkit-api.lorapok.tech`** (one label under the zone). Do **not** use `reportkit-api.lorapok.tech` — nested subdomains trigger a failing Advanced certificate. See [DOMAIN-CONTROL-PANEL-INSTRUCTIONS.md §0](./docs/DOMAIN-CONTROL-PANEL-INSTRUCTIONS.md).
...
### B. Move `lorapok.tech` DNS to Cloudflare (required for `reportkit-api`)
...
### C. Worker custom domain `reportkit-api.lorapok.tech`
...
2. **Workers & Pages** → **[reportkit-demo-api → Settings → Domains](https://dash.cloudflare.com/f049faaf2f67549f5c58837479596a4a/workers/services/view/reportkit-demo-api/production/settings)** → **Add custom domain** → **`reportkit-api.lorapok.tech`** → **Continue**.
...
## `reportkit-api` — Worker custom domain (step by step)
...
| Custom domain `reportkit-api.lorapok.tech` | **Cloudflare dashboard** (below) |
...
2. **Add** → **Custom domain** → enter `reportkit-api.lorapok.tech`
...
|------|------|---------|-------|
| CNAME | `reportkit-api` | `<worker-subdomain>.workers.dev` or target shown in Workers UI | Proxied |

> The exact target string is shown in the Worker **Custom domains** panel after you add `reportkit-api.lorapok.tech`. Do not guess — copy from the dashboard.
...
- **Production (after DNS live):** `https://reportkit-api.lorapok.tech` (set in `.github/workflows/deploy-site.yml` after TLS verify)
- **Temporary (current):** `https://reportkit-demo-api.mdshuvo40.workers.dev`
...
- **Custom domain:** `reportkit-api.lorapok.tech` (see section above)
...
curl -s "https://reportkit-api.lorapok.tech/v1/health"
curl -s "https://reportkit-api.lorapok.tech/v1/stats"
curl -s "https://reportkit-api.lorapok.tech/v1/data?start_date=2012-01-01&end_date=2017-12-31&start=0&length=5"
```
