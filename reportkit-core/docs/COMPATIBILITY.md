...
| `@lorapok-labs/reportkit-ui` | CSS / JS (CAS tokens, sync loader, table helpers) | — (browser) | Any host that serves static assets |
...
| Host stack | Use |
|------------|-----|
| PHP 5.6–7.x + Laravel 4.1 / 4.2 | `core` + `laravel-legacy` + `@lorapok-labs/reportkit-ui` |
| PHP 7.0–7.4 + Laravel 5.5 – 8 | `core` + `laravel` + `@lorapok-labs/reportkit-ui` |
| PHP 8.0–8.5 + Laravel 9 – 13 | `core` + `laravel` + `@lorapok-labs/reportkit-ui` |
